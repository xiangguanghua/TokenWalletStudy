...
import {DeployMoodNfts} from "../../script/nfts/DeployMoodNfts.s.sol";
...
contract MoodNftTest is Test /** , ZkSyncChainChecker, FoundryZkSyncChecker*/ {
    ...
    function testViewTokenURI() public {
        vm.prank(USER);
        moodNfts.mintNft();
        console.log(moodNfts.tokenURI(0));
    }

    function testInitializedCorrectly() public view {
        assert(
            keccak256(abi.encodePacked(moodNfts.name())) ==
                keccak256(abi.encodePacked((NFT_NAME)))
        );
        assert(
            keccak256(abi.encodePacked(moodNfts.symbol())) ==
    ...
    function testCanMintAndHaveABalance() public {
        ...
        assert(moodNfts.balanceOf(USER) == 1);
    }
    ...
    function testTokenURIDefaultIsCorrectlySet() public {
        ...
        assert(
            keccak256(abi.encodePacked(moodNfts.tokenURI(0))) ==
    ...
    function testEventRecordsCorrectTokenIdOnMinting() public {
        uint256 currentAvailableTokenId = moodNfts.getTokenCounter();
